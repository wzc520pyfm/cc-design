...
# 选择你的 AI 平台
npx cc-design init --ai claude    # Claude Code
npx cc-design init --ai cursor    # Cursor
npx cc-design init --ai codex     # Codex CLI
npx cc-design init --ai all       # 所有平台
```

### 全局安装（对所有项目生效）

```bash
npx cc-design init --ai claude --global
npx cc-design init --ai cursor --global
```
...
```bash
npx cc-design init --ai <platform> [--global]   # 安装
```
...
```bash
npx cc-design@latest init --ai <platform>
```
